...
/**
 * Shared fixture loading logic for Node (MockDataSource) and browser (BrowserDataSource).
 *
 * @abstract
 */
export class FixtureDataSource extends DataSource {
  /**
   * @param {(relativePath: string) => Promise<unknown>} loadFixture
   */
  constructor(loadFixture) {
    super();
  ...
  async loadAccountFixture(customerId, fileName) {
    return this.loadFixture(`accounts/${customerId}/${fileName}`);
  }
